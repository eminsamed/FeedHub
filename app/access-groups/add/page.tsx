...
export default function Page() {
  ...
  const handleAddUser = () => {
    setAccessGroup((prev) => ({
      ...prev,
      accessGroupUsers: [
        ...prev.accessGroupUsers,
        { email: "", userPrivilege: UserPrivilege.Viewer },
      ],
    }));
  };

  const handleUserChange = (index: number, field: string, value: any) => {
    const updatedUsers = accessGroup.accessGroupUsers.map((user, i) =>
          ...
          {accessGroup.accessGroupUsers.map((user, index) => (
            <Paper key={index} className="p-3 my-3" elevation={1}>
              ...
              <FormControl fullWidth margin="normal">
                ...
                <Select
                  value={user.userPrivilege}
                  onChange={(e) =>
                    handleUserChange(index, "userPrivilege", e.target.value)
                  }
                >
                  <MenuItem value={UserPrivilege.Viewer}>Viewer</MenuItem>
                  <MenuItem value={UserPrivilege.Member}>Member</MenuItem>
                  <MenuItem value={UserPrivilege.Administrator}>
                    Administrator
                  </MenuItem>
                </Select>
              </FormControl>
